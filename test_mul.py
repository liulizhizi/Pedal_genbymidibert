...
from main_0619_mul import *
from utils.b_reverse_fill import process_music_data, has_positive_value
from utils.gen_midi_type_xlsx import process_midi_df
from utils.a_midi_gen_new import process_music_df, apply_business_rules, transform_to_midi
...
def cat(predict_squeezed, input_data):
    ...
    - data_mask: 损失掩码 (batch_size, seq_length, features)
    ...
    clipped_p = torch.where(p < 2437, torch.tensor(0, device=p.device, dtype=p.dtype), p)
    clipped_p = torch.where(clipped_p > 2820, torch.tensor(2820, device=p.device, dtype=p.dtype), clipped_p)
    d = predict_squeezed[..., 1]
    clipped_d = torch.where(d < 157, torch.tensor(0, device=p.device, dtype=p.dtype), p)
    clipped_d = torch.where(clipped_d > 2436, torch.tensor(2436, device=p.device, dtype=p.dtype), clipped_p)
    ...
    # 组合掩码：基础掩码和动态掩码的交集
    combined_mask = torch.tensor(feature_mask_p & feature_mask_d, dtype=torch.bool)
    ...
    ).to(updated_input.dtype)

    updated_input[..., pedal_id] = torch.where(
        combined_mask.unsqueeze(-1),
        torch.tensor(2882, dtype=torch.int),
        torch.tensor(0, dtype=torch.int)
    ).to(updated_input.dtype)

    #检测超出范围的次数
    exceed_mask_p = combined_mask & ((predict_squeezed[..., 0] < 2437) | (predict_squeezed[..., 0] > 2820))
    exceed_mask_d = combined_mask & ((predict_squeezed[..., 0] < 157) | (predict_squeezed[..., 0] > 2436))

    # print(predict_squeezed.shape)
    exceed_p_count = torch.sum(exceed_mask_p).item()
    exceed_d_count = torch.sum(exceed_mask_d).item()
...
if __name__ == "__main__":
    ...
    for test_file in data["test"]:
        ...
        print(test_loss, loss_p, loss_d)
        print(test_acc, acc_p, acc_d)
        ...
        c_data = torch.cat((data_input, data_tar), dim=1)  # 形状 (n, 5 + 3)


        output, df, exceed_p_count, exceed_d_count= cat(data_pre, c_data)
